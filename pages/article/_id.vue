<!-- <template>
  <div>
    <Nav class="mx-auto sticky top-0" />
    {{ exactArticle }}
    <div class="w-4/5 sm:w-1/2 mx-auto my-5">
      <h3 class="my-5 font-bold text-4xl">
        {{ exactArticle.name }}
      </h3>
      <img
        :src="`http://localhost:1337${article.Image.url}`"
        class="max-h-screen"
      />
      <p class="mt-5 font-bold">
        written by {{ exactArticle.users_permissions_user.username }}
      </p>
      <div class="my-5">
        {{ exactArticle.content }}
      </div>
      <button
        v-if="
          $strapi.user &&
          exactArticle.users_permissions_user.id === $strapi.user.id
        "
        class="button--grey"
        @click="deletePost(article.id)"
      >
        Delete
      </button>
    </div>
  </div>
</template>
<script setup>
const exactArticle = await useAsyncData('exactArticle', () => {
  const id = route.params.id

  $fetch(
    this.$strapi.$articles.findOne(id, {
      populate: '*',
    })
  )

  return { article }
})

const deletePost = async (id) => {
  await this.$strapi.$articles.delete(id)
  this.$nuxt.$router.push('/articles')
}
</script>

<style scoped>
h1 {
  font-weight: 700;
  font-size: 2rem;
  margin: 0.5em 0;
}
</style> -->
